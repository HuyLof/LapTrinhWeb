...
function addToWishList(x) {
    var pNode = x.closest('.product__item');
    var prodImg = pNode.querySelector('.product__item__pic img');
    var urlImage = new URL(prodImg.src);
    var probText = pNode.querySelector('.product__item__text');
    var name = probText.querySelector('h6').innerText;
    var price = probText.querySelector('h5 span').innerText;
    var prob = [urlImage, name, price];
...
function removeToWishList(x) {
    ...
    for (var i = wishList.length - 1; i >= 0; i--) {
        ...
        if (arraysIdentical(element, prob)) {
            wishList.splice(i, 1);
        }
    }
